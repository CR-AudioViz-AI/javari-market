...
export async function GET(
  ...
  try {
    ...
    const currentPrice = marketIntel?.price?.current || finnhubIntel.quote?.price || 0;
    const sma50 = marketIntel?.technicals?.sma50 || 0;
    const sma200 = marketIntel?.technicals?.sma200 || 0;
    ...
    const response: StockIntelligenceResponse = {
      ...
      technicals: {
        ...
        macd: marketIntel?.technicals?.macd ? {
          value: marketIntel.technicals.macd.value,
          signal: marketIntel.technicals.macd.signal,
          histogram: marketIntel.technicals.macd.histogram,
        ...
        bollingerBands: marketIntel?.technicals?.bollingerBands ? {
          upper: marketIntel.technicals.bollingerBands.upper,
          middle: marketIntel.technicals.bollingerBands.middle,
          lower: marketIntel.technicals.bollingerBands.lower,
          position: (currentPrice > marketIntel.technicals.bollingerBands.upper) ? 'UPPER'
                   : (currentPrice < marketIntel.technicals.bollingerBands.lower) ? 'LOWER'
                   : 'MIDDLE',
        } : null,
      },
      ...
      // Map RiskMetrics (overallScore, riskLevel) to response format (score, level)
      risk: marketIntel?.risk ? {
        score: marketIntel.risk.overallScore,
        level: marketIntel.risk.riskLevel,
        factors: {
          volatility: marketIntel.risk.factors?.volatility || 50,
          liquidity: marketIntel.risk.factors?.liquidity || 50,
          marketCap: marketIntel.risk.factors?.marketCap || 50,
          newsVolatility: marketIntel.risk.factors?.newsVolatility || 50,
          technicalRisk: marketIntel.risk.factors?.technicalRisk || 50,
        },
        warnings: marketIntel.risk.warnings || [],
      } : {
        score: 50,
      ...
      news: [
        ...(marketIntel?.sentiment?.topHeadlines?.map(h => ({
          headline: h.title,
          summary: '',
          source: h.source,
